...
public class AdminConsoleCompatController {

    @Autowired private AdminAuth adminAuth;
    @Autowired private AppRegistryService appRegistryService;
    @Autowired private RegistryController registryController;
    @Autowired private SessionRegistry sessionRegistry;
    @Autowired private WmppRole role;
    @Autowired @Nullable private PusherClient pusherClient;
    @Autowired private SchedulerService schedulerService;
    @Autowired private PusherPoolService pusherPoolService;
    @Autowired private PushProgressService pushProgressService;
    @Autowired private PushTaskQueueService pushTaskQueueService;
    ...
    public AdminOverview overview(@RequestHeader(name = "X-Admin-Token", required = false) String token) {
        ...
        List<TenantRow> tenants = apps.stream().map(app -> new TenantRow(app.appId(), app.allowPush(), onlineFor(app.appId(), routes))).toList();
        ...
        return new AdminOverview(apps.size(), totalOnline, activeTenants, Math.max(1, nodeConnections.size()), pushTaskQueueService.queueSize(), role.name(), schedulerService.currentStrategyName(), concurrencyNote(nodeConnections), tenants, nodeConnections, routes, pusherPoolService.snapshot(nodeConnections), pushProgressService.snapshotAll());
    }
    ...
    private Map<String, Map<String, String>> snapshotRoutes() {
        if (role == WmppRole.mono) {
            ...
            for (AppRegistryService.AppInfo app : appRegistryService.list()) {
                ...
                for (int i = 1; i <= online; i++) users.put("online-" + i, "local");
    ...
    private int onlineFor(String appId, Map<String, Map<String, String>> routes) {
        if (role == WmppRole.mono) return sessionRegistry.getOnlineCount(appId);
    ...
    private Map<String, Integer> aggregateNodes(Map<String, Map<String, String>> routes) {
        ...
        for (String pusherId : pusherClient.getAllBaseUrls().keySet()) counts.put(pusherId, 0);
        for (Map<String, String> appRoutes : routes.values()) {
            if (appRoutes == null) continue;
            for (String pusherId : appRoutes.values()) if (pusherId != null && !pusherId.isBlank()) counts.merge(pusherId, 1, Integer::sum);
        }
        return counts;
    }

    private String concurrencyNote(Map<String, Integer> nodeConnections) {
        if (nodeConnections == null || nodeConnections.isEmpty()) return "当前暂无可观测节点负载数据";
        int total = nodeConnections.values().stream().mapToInt(Integer::intValue).sum();
        int nodeCount = nodeConnections.size();
        if (nodeCount <= 1) return "当前为单节点承载，可通过增加 pusher 实例演示横向扩展";
